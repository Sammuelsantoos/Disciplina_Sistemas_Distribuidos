...
class AdminClientTests(unittest.TestCase):
    ...
    def test_admin_client_cli_accepts_host_and_port(self):
        from src.client.admin_client import build_parser

        args = build_parser().parse_args(["--host", "127.0.0.1", "--port", "5005"])
...
class ViewerClientTests(unittest.TestCase):
    def test_viewer_client_cli_accepts_multicast_group_and_port(self):
        from src.client.viewer_client import build_parser

        args = build_parser().parse_args(["--group", "230.0.0.5", "--port", "6005"])

        self.assertEqual(args.group, "230.0.0.5")
        self.assertEqual(args.port, 6005)
