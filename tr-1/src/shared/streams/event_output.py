"""Escrita incremental e serialização de eventos em formato CSV para fluxos de saída.

Esta classe atua como um wrapper para despachar registros estruturados para múltiplos
destinos suportados, como arquivos locais, saída padrão ou sockets TCP conectados.
"""
...
class MatchEventOutputStream:
    """Gerencia o envio e serialização de eventos esportivos para fluxos de saída."""
    ...
    def __init__(self, destination_stream, event_array: List[MatchEvent], count: int):
        ...
        :param destination_stream: O objeto de destino para onde os dados serão enviados.
                                   Pode ser: sys.stdout, um arquivo aberto para escrita,
                                   ou um objeto de socket TCP conectado.
        :param event_array: Lista (array) contendo instâncias de objetos MatchEvent.
        :param count: O número exato de objetos que deverão ser enviados pelo stream.
        """
    ...
    def write_all(self) -> None:
        """
        Percorre a quantidade 'count' de objetos, serializa cada um deles para
        texto no formato CSV e despacha os bytes para o destino configurado.
        """
